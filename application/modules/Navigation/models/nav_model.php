...
class Nav_model extends CI_Model {
    ...
    public function getMenuID($menu_name) {
         ...
         * @param $menu_name : string name associated to menu defined in CI_Nav_Menus
         * @return int menu ID otherwise -1;
         */
        if (isset($menu_name) && ctype_alnum ($menu_name)) {
            $query = $this->db->query('SELECT `MenuID` FROM ci_nav_menus WHERE `MenuName` = ?',array($menu_name));
            $row = $query->row();
            return $row->MenuID;
        }
    ...
    public function getTopLevelNav_byID($menu_ID) {
        /**
         * Returns the top level of a menu
         * @param $menu_ID : int ID of menu as defined in CI_Nav_Menus
