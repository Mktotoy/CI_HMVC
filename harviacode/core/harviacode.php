...
class Harviacode
{
    ...
    function all_foreign_keys($table)
    {
        $query = "SELECT 
                     COLUMN_NAME,CONSTRAINT_NAME, REFERENCED_TABLE_NAME as TABLE_NAME,REFERENCED_COLUMN_NAME
                    FROM
        ...
        $stmt = $this->sql->prepare($query) OR die("Error code :" . $this->sql->errno . " (not_foreign fields)");
        $stmt->bind_param('ss', $this->database, $table);
        $stmt->bind_result($column_name, $constraint_name, $table_name,$referenced_column_name);
        $stmt->execute();
        while ($stmt->fetch()) {
            $fields[] = array('column_name' => $column_name, 'constraint_name' => $constraint_name, 'table_name' => $table_name,'referenced_column_name' => $referenced_column_name);
        }
        return $fields;
        $stmt->close();
        $this->sql->close();
